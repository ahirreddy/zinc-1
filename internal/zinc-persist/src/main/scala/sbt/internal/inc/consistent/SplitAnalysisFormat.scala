...
/** Split analysis format that serializes zinc's incremental state into two separate files:
 *
 * - A dependency file containing only the data needed for dependency analysis
 *   (relations.productClassName + apis.internal).
 * - A main file containing everything else (MiniSetup, remaining relations, stamps,
 *   apis.external, sourceInfos).
 *
 * Based on [[ConsistentAnalysisFormat]].
 */
class SplitAnalysisFormat(val mappers: ReadWriteMappers, reproducible: Boolean) {
  import SplitAnalysisFormat._
  ...
  def writeDep(out: Serializer, analysis: CompileAnalysis, storeApis: Boolean): Unit = {
    val analysis0 = analysis match { case analysis: Analysis => analysis }
    out.int(VERSION)
    out.bool(storeApis)
    writeDepRelations(out, analysis0.relations)
    writeDepAPIs(out, analysis0.apis, storeApis)
    out.int(VERSION)
    out.end()
  }

  def readDep(in: Deserializer): CompileAnalysis = {
    readVersion(in)
    val storeApis = in.bool()
    val productClassName = readDepRelations(in)
    val internalApis = readDepAPIs(in, storeApis)
    readVersion(in)
    in.end()
    Analysis.Empty.copy(
      stamps = Stamps.empty,
      apis = APIs(internalApis, Map.empty),
      relations = Relations.empty.copy(productClassName = productClassName),
      infos = SourceInfos.empty,
      compilations = Compilations.of(Nil)
    )
  }

  // ===== Main file =====

  def writeMain(out: Serializer, analysis: CompileAnalysis, setup: MiniSetup): Unit = {
    val analysis0 = analysis match { case analysis: Analysis => analysis }
    out.int(VERSION)
    writeMiniSetup(out, setup)
    writeMainRelations(out, analysis0.relations)
    writeStamps(out, analysis0.stamps)
    writeMainAPIs(out, analysis0.apis, setup.storeApis())
    writeSourceInfos(out, analysis0.infos)
    out.int(VERSION)
    out.end()
  }

  def readMain(in: Deserializer, depAnalysis: CompileAnalysis): (CompileAnalysis, MiniSetup) = {
    val dep = depAnalysis match { case a: Analysis => a }
    readVersion(in)
    val setup = readMiniSetup(in)
    val mainRelations = readMainRelations(in)
    val stamps = readStamps(in)
    val externalApis = readMainAPIs(in, setup.storeApis())
    val infos = readSourceInfos(in)
    val compilations = Compilations.of(Nil)
    readVersion(in)
    in.end()
    val relations = mainRelations.copy(productClassName = dep.relations.productClassName)
    val apis = APIs(dep.apis.internal, externalApis)
  ...
  private[this] def writeAPIMap(
      out: Serializer,
      name: String,
      m: Map[String, AnalyzedClass],
      storeApis: Boolean
  ): Unit = {
    writeMaybeSortedStringMap(
      out,
  ...
  private[this] def readAPIMap(
      in: Deserializer,
      storeApis: Boolean
  ): Map[String, AnalyzedClass] = {
    in.readColl[(String, AnalyzedClass), Map[String, AnalyzedClass]](Map, 2) {
      (in.string(), readAnalyzedClass(in, storeApis))
    }
  }

  private[this] def writeDepAPIs(out: Serializer, apis: APIs, storeApis: Boolean): Unit =
    writeAPIMap(out, "internal", apis.internal, storeApis)

  private[this] def readDepAPIs(in: Deserializer, storeApis: Boolean): Map[String, AnalyzedClass] =
    readAPIMap(in, storeApis)

  private[this] def writeMainAPIs(out: Serializer, apis: APIs, storeApis: Boolean): Unit =
    writeAPIMap(out, "external", apis.external, storeApis)

  private[this] def readMainAPIs(
      in: Deserializer,
      storeApis: Boolean
  ): Map[String, AnalyzedClass] =
    readAPIMap(in, storeApis)
  ...
  private[this] def writeDepRelations(out: Serializer, rs: Relations): Unit = {
    def wrS(name: String, rel: Relation[String, String]): Unit =
      writeMaybeSortedStringMap(
        out,
        name,
        rel.forwardMap.view.map { case (k, vs) => k -> vs }
      ) { vs =>
        val a = vs.iterator.map(identity[String]).toArray
  ...
  private[this] def readDepRelations(in: Deserializer): Relation[String, String] = {
    def rdS(): Relation[String, String] =
      Relation.reconstruct(in.readColl[(String, Set[String]), Map[String, Set[String]]](Map, 2) {
        (in.string(), in.readColl[String, Set[String]](Set) { in.string() })
      })
    rdS()
  }

  private[this] def writeMainRelations(out: Serializer, rs: Relations): Unit = {
  ...
  private[this] def readMainRelations(in: Deserializer): Relations = {
    val un =
    ...
    val mri, mre, ii, ie, lii, lie, mei, mee = rdS()
    def deps(
...
object SplitAnalysisFormat {
  ...
  private[consistent] val emptySetup: MiniSetup = MiniSetup.of(
    CompileOutput.empty,
